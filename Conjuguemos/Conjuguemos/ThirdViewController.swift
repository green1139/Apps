...
class ThirdViewController: UIViewController, UIWebViewDelegate {
    ...
    @IBAction func goHome(sender: AnyObject) {
        let url = NSBundle.mainBundle().URLForResource("grammar\(languageToLearn)", withExtension:"html")
        if (url != nil)
        {
    ...
    override func viewDidAppear(animated: Bool)
    {
        ...
        {
            NSUserDefaults.standardUserDefaults().setObject(highScore, forKey: "grammarHighScore")
            self.navigationController?.tabBarItem.badgeValue = "\(highScore)%"
        }
        else
        {
            highScore = NSUserDefaults.standardUserDefaults().objectForKey("grammarHighScore") as! Double
            self.navigationController?.tabBarItem.badgeValue = "\(highScore)%"
        }
    ...
    func check()
    {
        ...
        if let data = data {
            if (true)///data == "0")
            {
                ...
                let data2 = webView.stringByEvaluatingJavaScriptFromString("allright")
                let data3 = webView.stringByEvaluatingJavaScriptFromString("attempted")!
                print("correct: \(data2!) and attempted: \(data3)")
                if let data2 = data2{
                    if (data2 != "" && data3 != "" && data3 != "0")
                    {
                        addNewScoreEntry("Grammar (\(languageToLearn))", title: title, score: data2+"/"+data3)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        /**if (segue.identifier == "grammarAlert")
        {
