...
class SecondViewController: UIViewController, UIWebViewDelegate {
    ...
    @IBAction func goHome(sender: AnyObject) {
        let url = NSBundle.mainBundle().URLForResource("conjugations\(languageToLearn)", withExtension:"html")
        if (url != nil)
        {
    ...
    override func viewDidAppear(animated: Bool)
    {
        ...
        {
            NSUserDefaults.standardUserDefaults().setObject(highScore, forKey: "conjugationsHighScore")
            self.navigationController?.tabBarItem.badgeValue = "\(highScore)%"
        }
        else
        {
            highScore = NSUserDefaults.standardUserDefaults().objectForKey("conjugationsHighScore") as! Double
            self.navigationController?.tabBarItem.badgeValue = "\(highScore)%"
        }
        ...
        if let data = data {
            if (data == "0")
            {
                ...
                if let data2 = data2{
                    ...
                    {
                        addNewScoreEntry("Conjugation (\(languageToLearn))", title: title, score: data2+"/"+data3)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        /**if (segue.identifier == "conjugationsAlert")
        {
